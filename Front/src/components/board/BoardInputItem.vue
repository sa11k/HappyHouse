    ...
    <b-col style="text-align: left; margin-left: 10%; margin-right: 10%">
      <b-form>
          ...
          <b-col>
            <b-row>
              ...
              <b-col cols="9" style="padding: 0">
                <b-form-input id="user_id" :disabled="isUserid" v-model="userInfo.id" type="text" readonly style="width: 95%"></b-form-input>
              </b-col>
            </b-row>
          </b-col>
        </b-row>
        ...
        <b-form-group id="content-group" label="내용" label-for="content">
          <b-form-textarea id="content" v-model="board.content" placeholder="내용 입력..." rows="10" max-rows="15" style="width: 98%; margin-left: 10px; margin-bottom: 20px"></b-form-textarea>
        </b-form-group>
        <hr />
        <b-row>
...
export default {
  ...
  created() {
    if (this.type === "modify") {
      http.get(`/board/updateform/${this.$route.params.no}`).then(({ data }) => {
        this.board = data;
      });
      this.isUserid = true;
    }
  ...
  methods: {
    onSubmit() {
      let err = true;
      let msg = "";
      !this.board.title && ((msg = "제목 입력해주세요"), (err = false));
      err && !this.board.content && ((msg = "내용 입력해주세요"), (err = false));
    ...
    registBoard() {
      const board = { title: this.board.title, content: this.board.content, user_id: this.userInfo.id, user_name: this.userInfo.name };
      http.post(`/board/register`, null, { params: board }).then(({ data }) => {
